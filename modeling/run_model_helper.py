...
def summarize_sections(full_doc_name, in_comp_dirpath, out_comp_dirpath, cluster_sizes,
                       model_func: Callable[[str, int], List[str]]):
    """
    ...
    for root, dirs, files in os.walk(in_comp_dirpath):
        for filename in files:
            if full_doc_name in filename:  # only process sections from this 10Q
                ...
                summarized_lines = model_func(doc, len(
                    doc.splitlines()) / 2)  # rank half the doc sentences, we'll cut down later
...
def summarize_docs(modifier, model_func: Callable[[str, int], List[str]], model_name: str):
    """
        ...
        for comp_name in dirs:
            ...
            for root2, dirs2, files2 in os.walk(in_comp_dirpath):
                for filename in files2:
                    ...
                    summarize_sections(full_doc_name, in_comp_dirpath, out_comp_dirpath, cluster_sizes, model_func)
